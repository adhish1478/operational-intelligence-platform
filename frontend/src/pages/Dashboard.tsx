...
export const Dashboard: React.FC = () => {
  ...
  const activeInvestigations = mockInvestigations.filter(inv => inv.status !== 'resolved');
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col gap-1 border-b border-outline-variant pb-4">
        ...
        <p className="text-body-md text-on-surface-variant">
          Hello Adhish. There are <strong className="text-error font-semibold">{activeInvestigations.length} active investigations</strong> requiring immediate triage.
        </p>
      </div>
              ...
              <div className="w-full md:w-[280px] bg-surface-low border-l border-outline-variant/60 pl-0 md:pl-5 pt-4 md:pt-0 flex flex-col justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-body-sm font-semibold text-on-surface flex items-center gap-1.5">
                    ...
                    {inv.category === 'customer_escalation' ? 'TechCorp Contract Renewal Blocked' : 'V2 Release Delayed (4 days)'}
                  </div>
                  <p className="text-[11px] text-on-surface-variant">
                    Suggested action: <span className="text-on-surface font-medium">{inv.suggestedAction}</span>
                  </p>
                </div>
