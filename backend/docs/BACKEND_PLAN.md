...
*   **Checklist:**
    ...
    *   [x] Implement database CRUD operations in `organizations/services.py` (get by ID, get by slug, list, create, invite/add member, update role, remove member).
    *   [x] Implement a global `ActiveOrganizationDep` FastAPI dependency to extract, validate, and inject the current organization tenant boundary using the `X-Organization-ID` header.
    *   [x] Connect service methods to API route handlers in `organizations/routes.py`.
    *   [x] Add integration test coverage validating tenant isolation (e.g., User A cannot view Organization B's details).
